	...
	for key, tokenPat := range tokenPats {
		...
		for _, token := range tokens {
			...
			if verify {
				...
				verified, err, _ := verifyMatch(ctx, client, token)
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, client *http.Client, token string) (bool, error, map[string]string) {
	...
	if err == nil {
		...
		if authResponse.Ok {
			extraData["team"] = authResponse.Team
			extraData["name"] = authResponse.User
