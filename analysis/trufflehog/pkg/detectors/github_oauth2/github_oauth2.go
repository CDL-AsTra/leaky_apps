...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, idMatch := range oauth2ClientIDMatches {
		...
		for _, secretMatch := range oauth2ClientSecretMatches {
			...
			if verify {
				verified, err := verifyMatch(ctx, idMatch[1], secretMatch[1])
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, idMatch, secretMatch string) (bool, error) {
	config := &clientcredentials.Config{
		ClientID:     idMatch,
		ClientSecret: secretMatch,
