...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			verified, err, result := verifyMatch(ctx, client, resMatch)
			if err != nil {
				continue
			}
			s1.Verified = verified
			s1.AnalysisInfo = result
		}
...
func verifyMatch(ctx context.Context, client *http.Client, secret string) (bool, error, map[string]string) {
	...
	if err == nil {
		...
		if res.StatusCode >= 200 && res.StatusCode < 300 {
			return true, nil, map[string]string{"key": secret}
