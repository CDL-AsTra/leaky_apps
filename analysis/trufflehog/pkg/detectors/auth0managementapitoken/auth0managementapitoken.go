	...
	for _, managementApiTokenMatch := range managementAPITokenMatches {
		...
		for _, domainMatch := range domainMatches {
			...
			if verify {
				...
				*/
				verified, err := verifyMatch(ctx, client, managementAPITokenRes, domainRes)
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, client *http.Client, managementAPITokenRes, domainRes string) (bool, error) {
