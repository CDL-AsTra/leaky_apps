	...
	for _, urlMatch := range urlMatches {
		...
		for _, secretMatch := range secretMatches {
			...
			s1 := detectors.Result{
				...
				Raw:          []byte(endpoint),
				RawV2:        []byte(endpoint + ";-|" + accountKey + ";-|" + accountName),
				Redacted:     endpoint,
			}
			...
			if verify {
				...
				verified, err := verifyMatch(ctx, client, endpoint, accountKey, accountName)
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, client *http.Client, endpoint, accountKey, accountName string) (bool, error) {
