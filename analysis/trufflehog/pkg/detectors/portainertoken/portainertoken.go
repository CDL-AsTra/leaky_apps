	...
	for _, match := range matches {
		...
		for _, endpointMatch := range endpointMatches {
			...
			if verify {
				...
				verified, err := verifyMatch(ctx, client, resMatch, u.String())
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, client *http.Client, resMatch, u string) (bool, error) {
