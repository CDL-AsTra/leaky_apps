	...
	for _, instance := range instanceMatches {
		...
		for _, token := range tokenMatches {
			...
			s1 := detectors.Result{
				...
				Raw:          []byte(tokenMatch),
				RawV2:        []byte(tokenMatch + instanceMatch),
			}
			...
			if verify {
				...
				verified, err := verifyMatch(ctx, client, tokenMatch, instanceMatch)
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, client *http.Client, tokenMatch, instanceMatch string) (bool, error) {
