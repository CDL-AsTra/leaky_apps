	...
	for _, match := range matches {
		...
		for _, idMatch := range idMatches {
			...
			if verify {
				...
				isVerified, verificationErr := verifyBraintree(ctx, client, url, resMatch, resIdMatch)
...
func (s Scanner) Verify(ctx context.Context, secret string) bool {
	...
	verified, _ := verifyBraintree(ctx, s.getClient(), url, parts[0], parts[1])
	return verified
}
...
func verifyBraintree(ctx context.Context, client *http.Client, url, privKey, pubKey string) (bool, error) {
