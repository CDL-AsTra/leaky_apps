	...
	for resAccountMatch := range uniqueAccountMatches {
		for _, resUsernameMatch := range usernameMatches {
			for _, resPasswordMatch := range passwordMatches {
				...
				if verify {
					verified, err, extraData := verifyMatch(ctx, uri)
					if err != nil {
						continue
					}
...
func (s Scanner) Verify(ctx context.Context, secret string) bool {
	verified, _, _ := verifyMatch(ctx, secret)
	return verified
}

func verifyMatch(ctx context.Context, resMatch string) (bool, error, map[string]string) {
	extraData := map[string]string{}
	// Open a connection to Snowflake
	db, err := sql.Open("snowflake", resMatch) // Needs the snowflake driver from gosnowflake
	if err != nil {
	...
	if err != nil {
		...
		extraData["databases"] = strings.Join(databases, ", ")
