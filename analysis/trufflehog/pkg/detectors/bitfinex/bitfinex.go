...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, apiKeyMatch := range apiKeyMatches {
		...
		for _, apiSecretMatch := range apiSecretMatches {
			...
			seretMatchString = seretMatchString + "," + apiSecretRes

			if verify {
				verified, err := verifyMatch(ctx, client, apiKeyRes, apiSecretRes)
				if err != nil {
					continue
				}
		...
		s1.RawV2 = []byte(apiKeyRes + ";-|" + seretMatchString)
...
func verifyMatch(ctx context.Context, client *http.Client, apiKeyRes, apiSecretRes string) (bool, error) {
...
func (s Scanner) Verify(ctx context.Context, secret string) bool {
	...
	for _, pvalue := range strings.Split(parts[1], ",") {

		verified, _ := verifyMatch(ctx, client, parts[0], pvalue)
		if verified {
			return verified
		}
	}
