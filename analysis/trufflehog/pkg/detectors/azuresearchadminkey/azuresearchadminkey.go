	...
	for _, match := range matches {
		...
		for _, serviceMatch := range serviceMatches {
			...
			if verify {
				...
				verified, err := verifyMatch(ctx, client, resMatch, resServiceMatch)
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, client *http.Client, resMatch, resServiceMatch string) (bool, error) {
