		...
		for _, clientSecretMatch := range clientSecretMatches {
			...
			for _, domainMatch := range domainMatches {
				...
				s1 := detectors.Result{
					...
					RawV2:        []byte(clientIdRes + ";-|" + clientSecretRes + ";-|" + domainRes),
				}
				...
				if verify {
					...
					*/
					verified, err := verifyMatch(ctx, client, clientIdRes, clientSecretRes, domainRes)
					if err != nil {
...
func verifyMatch(ctx context.Context, client *http.Client, clientIdRes, clientSecretRes, domainRes string) (bool, error) {
	data := url.Values{}
