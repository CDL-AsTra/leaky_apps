...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		if verify {
			verified, err := verifyAiven(ctx, client, resMatch)
			if err != nil {
				continue
			}
...
func verifyAiven(ctx context.Context, client *http.Client, resMatch string) (bool, error) {
	req, err := http.NewRequestWithContext(ctx, "GET", "https://api.aiven.io/v1/project", nil)
	if err != nil {
		return false, err
	}
...
func (s Scanner) Verify(ctx context.Context, secret string) bool {
	verified, _ := verifyAiven(ctx, client, secret)
	return verified
}
