...
var (
	...
	keyPat = regexp.MustCompile(`\b(ops_eyJ[a-zA-Z0-9+/]{250,}={0,3})\b`)
)
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		s1 := detectors.Result{
			DetectorType: detectorspb.DetectorType_1Password,
			Raw:          []byte(resMatch),
		}
...
func (s Scanner) Type() detectorspb.DetectorType {
	return detectorspb.DetectorType_1Password
}

func (s Scanner) Description() string {
	return "Uncovered a possible 1Password service account token, potentially compromising access to secrets in vaults."
}
