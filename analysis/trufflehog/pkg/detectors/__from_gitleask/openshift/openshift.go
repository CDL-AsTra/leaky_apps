...
var (
	...
	keyPat = regexp.MustCompile(detectors.PrefixRegex([]string{"openshift"}) + `\b(sha256~[\w-]{43})\b`)
)
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		s1 := detectors.Result{
			DetectorType: detectorspb.DetectorType_Openshift,
			Raw:          []byte(resMatch),
		}
...
func (s Scanner) Type() detectorspb.DetectorType {
	return detectorspb.DetectorType_Openshift
}

func (s Scanner) Description() string {
	return "Found an OpenShift user token, potentially compromising an OpenShift/Kubernetes cluster."
}
