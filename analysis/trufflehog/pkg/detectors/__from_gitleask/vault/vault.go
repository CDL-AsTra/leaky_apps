...
var (
	...
	keyPat = regexp.MustCompile(`\b(hvb\.[a-z0-9_-]{138,212})\b`)
)
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		s1 := detectors.Result{
			DetectorType: detectorspb.DetectorType_Vault,
			Raw:          []byte(resMatch),
		}
...
func (s Scanner) Type() detectorspb.DetectorType {
	return detectorspb.DetectorType_Vault
}

func (s Scanner) Description() string {
	return "Detected a Vault Batch Token, risking unauthorized access to secret management services and sensitive data."
}
