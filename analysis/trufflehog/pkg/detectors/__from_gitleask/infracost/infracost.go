...
var (
	...
	keyPat = regexp.MustCompile(`\b(ico-[a-zA-Z0-9]{32})\b`)
)
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		s1 := detectors.Result{
			DetectorType: detectorspb.DetectorType_Infracost,
			Raw:          []byte(resMatch),
		}
...
func (s Scanner) Type() detectorspb.DetectorType {
	return detectorspb.DetectorType_Infracost
}

func (s Scanner) Description() string {
	return "Detected an Infracost API Token, risking unauthorized access to cloud cost estimation tools and financial data."
}
