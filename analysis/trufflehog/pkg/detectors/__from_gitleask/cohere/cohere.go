...
var (
	...
	keyPat = regexp.MustCompile(detectors.PrefixRegex([]string{"cohere"}) + `\b([a-zA-Z0-9]{40})\b`)
)
...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, match := range matches {
		...
		s1 := detectors.Result{
			DetectorType: detectorspb.DetectorType_Cohere,
			Raw:          []byte(resMatch),
		}
...
func (s Scanner) Type() detectorspb.DetectorType {
	return detectorspb.DetectorType_Cohere
}

func (s Scanner) Description() string {
	return "Identified a Cohere Token, posing a risk of unauthorized access to AI services and data manipulation."
}
