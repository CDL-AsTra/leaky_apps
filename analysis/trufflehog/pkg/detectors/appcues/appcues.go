		...
		for _, userMatch := range userMatches {
			...
			for _, idMatch := range idMatches {
				...
				s1 := detectors.Result{
					...
					Raw:          []byte(resMatch),
					RawV2:        []byte(resMatch + ";-|" + resUserMatch + ";-|" + resIdMatch),
				}
				if verify {
					verified, err := verifyMatch(ctx, client, resMatch, resUserMatch, resIdMatch)
					if err != nil {
						continue
					}
...
func verifyMatch(ctx context.Context, client *http.Client, resMatch, resUserMatch, resIdMatch string) (bool, error) {
...
func (s Scanner) Verify(ctx context.Context, secret string) bool {
	...
	verified, _ := verifyMatch(ctx, client, parts[0], parts[1], parts[3])
	return verified
