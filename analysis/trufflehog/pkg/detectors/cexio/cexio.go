		...
		for _, keyMatch := range keyMatches {
			...
			for _, secretMatch := range secretMatches {
				...
				s1 := detectors.Result{
					...
					RawV2:        []byte(resKeyMatch + ";-|" + resUserIdMatch + ";-|" + resSecretMatch),
				}

				if verify {
					verified, err := verifyMatch(ctx, client, resKeyMatch, resUserIdMatch, resSecretMatch)
					if err != nil {
						continue
					}
...
func verifyMatch(ctx context.Context, client *http.Client, resKeyMatch, resUserIdMatch, resSecretMatch string) (bool, error) {
