...
func (s Scanner) FromData(ctx context.Context, verify bool, data []byte) (results []detectors.Result, err error) {
	...
	for _, apiMatch := range apiMatches {
		...
		for _, appMatch := range appMatches {
			...
			if verify {
				verified, err := s.verifyMatch(ctx, client, resAppMatch, resApiMatch)
				if err != nil {
					continue
				}
		...
		if !appIncluded {
			...
			if verify {
				verified, err := s.verifyMatch(ctx, client, "", resApiMatch)
				if err != nil {
					continue
				}
				s1.Verified = verified
...
func (s Scanner) Verify(ctx context.Context, secret string) bool {
	...
	verified, _ := s.verifyMatch(ctx, client, appMatch, apiMatch)
	return verified
}
...
func (s Scanner) verifyMatch(ctx context.Context, client *http.Client, resAppMatch, resApiMatch string) (bool, error) {
	if resAppMatch != "" {
