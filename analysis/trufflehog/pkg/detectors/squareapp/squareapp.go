	...
	for _, match := range matches {
		for _, secMatch := range secMatches {
			...
			if verify {
				...
				verified, err := verifyMatch(ctx, client, match, secMatch)
				if err != nil {
					continue
				}
...
func verifyMatch(ctx context.Context, client *http.Client, match, secMatch string) (bool, error) {
